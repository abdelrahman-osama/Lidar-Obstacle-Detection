...
typename pcl::PointCloud<PointT>::Ptr ProcessPointClouds<PointT>::FilterCloud(typename pcl::PointCloud<PointT>::Ptr cloud, float filterRes, Eigen::Vector4f minPoint, Eigen::Vector4f maxPoint)
{
    ...
    roof.setMin(Eigen::Vector4f(-1.5, -1.7, -1, 2));
...
static void clusterHelper(int i, const std::vector<std::vector<float>> &points, std::vector<int> &cluster, std::vector<bool> &processed, KdTree *tree, float distanceTol)
{
    processed[i] = true;
    cluster.push_back(i);
...
template <typename PointT>
std::vector<typename pcl::PointCloud<PointT>::Ptr> ProcessPointClouds<PointT>::EKClustering(typename pcl::PointCloud<PointT>::Ptr cloud, float clusterTolerance, int minSize, int maxSize)
{
    ...
    for (auto clusterIndex : clusterIndices)
    {
        ...
        // std::vector<int> clusterI = clusterIndices[i];
        for (auto pointI : clusterIndex)
        {
            singleCluster->points.push_back(cloud->points[pointI]);
        }
        singleCluster->width = singleCluster->points.size();
        singleCluster->height = 1;
        singleCluster->is_dense = true;

        if (singleCluster->points.size() >= minSize && singleCluster->points.size() <= maxSize)
